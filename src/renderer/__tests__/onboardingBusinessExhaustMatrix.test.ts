...
describe('onboarding business exhaust matrix (App snapshot contract)', () => {
  ...
  it("c) setup with goal confirmed and no participants → pick_participants, not complete", () => {
    const { stepId, complete } = resolveBusiness({
      brainstormView: 'setup',
  ...
  it("h) live room after speak with the whole tour done → null and complete", () => {
    const { stepId, complete } = resolveBusiness({
      brainstormView: 'r1',
      brainstormRooms: [{ id: 'r1' }],
      liveRoomIds: ['r1'],
      humanSpokeByRoom: { r1: true },
      doneSteps: ['stop_room', 'finish_room', 'saved_rooms'],
    })
    expect(stepId).toBe(null)
