...
export const AgentConfigModal: React.FC<AgentConfigModalProps> = ({
  ...
  const [cliStatuses, setCliStatuses] = useState<AgentCliStatusMap>({})
  ...
  useEffect(() => {
    if (!open) return
    let cancelled = false
    void window.api.detectAgentClis().then(statuses => {
      if (!cancelled) setCliStatuses(statuses)
    }).catch(() => { /* sin detección: la rejilla no afirma nada */ })
    return () => { cancelled = true }
  }, [open])
    ...
    const chips: AgentConfigHeroChip[] = [
      {
        ...
        tone: cliStatuses[meta.provider]?.found === false ? 'warn' : 'default',
        section: 'engine',
        title: cliStatuses[meta.provider]?.found === false
