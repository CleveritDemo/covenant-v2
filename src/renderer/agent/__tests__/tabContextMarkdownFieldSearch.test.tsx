...
describe('TabContextMarkdownField — búsqueda en cuerpo', () => {
  ...
  it('sin coincidencias muestra el mensaje vacío y deshabilita la navegación', () => {
    ...
    expect((screen.getByRole('button', { name: 'tabContexts.bodySearchPrev' }) as HTMLButtonElement).disabled).toBe(true)
    expect((screen.getByRole('button', { name: 'tabContexts.bodySearchNext' }) as HTMLButtonElement).disabled).toBe(true)
  })
})
