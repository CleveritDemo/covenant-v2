...
function renderSetupPlane(overrides: Partial<TabAgenticPlaneProps> = {}): ReturnType<typeof render> {
  return render(
    <TabAgenticPlane
      {...baseProps}
...
describe('coach mark write_goal sobre BrainstormStartModal real', () => {
  it('write_goal: ancla, coach mark, aro en el ancla y messageKey', () => {
    stubAnchorRects()
    ...
    expect(document.querySelector('.onboarding-coach-target')).toBeTruthy()
    expect(document.body.textContent).toContain('tabs.onboardingGuide.writeGoal')
  })

  it('write_goal con tabActive false: no monta coach mark', () => {
    stubAnchorRects()
  ...
  it('pick_participants: ancla y coach mark montados', () => {
    stubAnchorRects()
    ...
    expect(document.querySelector('[data-onboarding="brainstorm-participants"]')).toBeTruthy()
    expect(document.querySelector('.onboarding-coach-mark')).toBeTruthy()
  })

  it('cambio de ancla: create_team → write_goal remide al montar el modal', () => {
    stubAnchorRects()
