...
describe('buildBrainstormTurnPrompt', () => {
  it('includes objective, speaker labels, and brief plain-reply constraints', () => {
    ...
    // Sin techo de palabras ni veto a las tools: la sala usa MCP si hace falta.
    expect(prompt).not.toMatch(/≤50 words|<=50 words/i)
    expect(prompt).not.toMatch(/Do not delegate, call tools/)
