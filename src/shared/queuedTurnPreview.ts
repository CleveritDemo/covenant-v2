...
import { isReplicaAgentId } from './orchestrationAwaiting'
import {
...
export function resolveAgentLabel(
  ...
  const replica = isReplicaAgentId(to)
  const tag = replica ? agentInstanceTag(to) : null
  const expertId = (tag ? parseExpertReplicaRequest(to).baseId : to) || to
  const agentLabel = catalog.length
    ? formatCatalogAgentDelegationLabel(expertId, catalog)
    : expertId
  return {
    agentLabel,
    ...(tag ? { instanceTag: tag } : {}),
